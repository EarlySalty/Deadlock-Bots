...
pub(crate) struct GatewayFreshness {
    shard: AtomicU32,
    ready: AtomicBool,
}
...
impl GatewayFreshness {
    ...
    pub(crate) fn resume(&self, shard: u32) {
        if self.shard.load(Ordering::Acquire) == shard {
            self.ready.store(true, Ordering::Release);
        }
    }
    pub(crate) fn stage(&self, shard: u32, connected: bool) {
        if !connected && self.shard.load(Ordering::Acquire) == shard {
...
fn role_rank(name: &str) -> Option<f64> {
    ...
    let (index, tier) = tokens.iter().enumerate().find_map(|(i, token)| {
        RANKS
            .iter()
            .position(|r| r == token)
            .map(|tier| (i, tier + 1))
...
pub(crate) fn directory(
    adapter: &crate::adapter::DiscordAdapter,
    guild_id: u64,
    user_id: u64,
    ...
    let guild = adapter
        ...
        .ok_or(PortError::GuildNotFound)?;
    let requester = guild
        .members
        .get(&UserId::new(user_id))
        .ok_or(PortError::MemberNotFound)?;
    ...
    for channel in guild.channels.values() {
        ...
        if !eligible(
            channel.id.get(),
            channel.parent_id.map(|p| p.get()),
            permissions,
        ) {
            continue;
        }
        // Timed-out members cannot connect even if cached role permissions allow it.
        if requester.communication_disabled_until.is_some_and(|until| {
        ...
        let brawl = channel
            .parent_id
            .is_some_and(|id| id.get() == CATEGORIES[2]);
        let competitive = channel
            .parent_id
            .is_some_and(|id| id.get() == CATEGORIES[1]);
        result.push(CommunityLobby {
            ...
            intent: (!streamer)
                .then(|| if competitive { "competitive" } else { "casual" }.to_string()),
            rank_average: (!ranks.is_empty())
                .then(|| ranks.iter().sum::<f64>() / ranks.len() as f64),
            rank_samples: ranks.len(),
            requester_present: members.iter().any(|member| member.user.id.get() == user_id),
            is_streamer_vc: streamer,
        });
    }
    result.sort_by(|a, b| {
        b.is_streamer_vc
            .cmp(&a.is_streamer_vc)
