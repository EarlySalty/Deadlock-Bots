...
mod tests {
    ...
    static SENKE_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());

    async fn senke_exklusiv() -> tokio::sync::MutexGuard<'static, ()> {
        SENKE_LOCK.lock().await
    }

    #[tokio::test]
    async fn ohne_token_wird_nichts_gestartet_und_keine_senke_registriert() {
        let _guard = senke_exklusiv().await;
