...
## Worum geht es?
Diese Doku fasst die Voice-Funktionen zusammen, die du direkt im Server merkst: TempVoice-Lanes, automatische Lane-Verteilung, Rang- und Statuslogik, Team-Balancing, Voice-Statistiken und die Steam-Link-Erinnerung per DM. Ziel ist, dass du schneller in passende Runden kommst und deine Lane ohne Moderation selbst steuern kannst. `Voice Reaction DM` ist dabei kein normales Community-Feature, sondern nur eine interne Benachrichtigung im Hintergrund.
...
- **Team-Balancer:** Sitze mit den Leuten im gleichen Voice und nutze `!balance auto` für die Vorschau oder `!balance start`, um zwei Teams zu bauen und direkt in Match-Voices zu verschieben. Dazu kommen `!balance manual`, `!balance status`, `!balance matches` und `!balance end <id>`.
...
- `!balance` braucht mindestens 4 Leute im aktuellen Voice. Bei mehr als 12 Leuten kommt erst eine Auswahlmaske.
...
- `cogs/deadlock_team_balancer.py`: Match-Balancer und Team-VC-Erstellung per `!balance`. Nutzt Turnier-Store und Rangquellen. Tabellen: Turnier-Store plus externe Rangdaten.
